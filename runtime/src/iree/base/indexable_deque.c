...
iree_status_t iree_indexable_queue_push_back(iree_indexable_queue_t* queue,
  ...
  if (queue->capacity == queue->count) {
    ...
    queue->capacity = iree_max(16, queue->capacity << 1);
        IREE_RETURN_IF_ERROR(iree_allocator_malloc(queue->element_size * queue->capacity);
        memcpy(new_mem, queue->elements + (queue->head * queue->element_size), (queue->count - queue->head) * queue->element_size);
        memcpy(new_mem + ((queue->count - queue->head) * queue->element_size), queue->elements, queue->head * queue->element_size);
        if (queue->elements != &queue->initial_allocation[0]) {
      iree_allocator_free(queue->alloactor, queue->elements);
        }
        queue->head = 0;
        queue->elements = new_mem;
  }
  memcpy(queue->elements + ((queue->head + queue->count) * queue->element_size),
         queue->element_size);
  queue->element_count++;
