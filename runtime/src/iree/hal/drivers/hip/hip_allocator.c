...
typedef struct iree_hal_hip_allocator_t {
  ...
  hipStream_t streams[IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT];
...
iree_status_t iree_hal_hip_allocator_create(
    const iree_hal_hip_dynamic_symbols_t* hip_symbols, uint32_t device_count,
    hipCtx_t* contexts, hipDevice_t* devices, hipStream_t* streams,
...
static iree_status_t iree_hal_hip_allocator_allocate_buffer(
  ...
  status = IREE_HIP_RESULT_TO_STATUS(
      allocator->symbols, hipCtxPushCurrent(allocator->contexts[device_idx]));
