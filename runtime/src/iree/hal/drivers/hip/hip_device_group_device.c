...
typedef struct iree_hal_hip_device_group_device_device_context_t {
...
} iree_hal_hip_device_group_device_device_context_t;
...
typedef struct iree_hal_hip_device_group_device_t {
  ...
  iree_hal_hip_device_group_device_device_context_t
      device_contexts[IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT];
} iree_hal_hip_device_group_device_t;
...
static iree_status_t iree_hal_hip_device_group_device_create_internal(
  ...
  iree_hal_hip_device_group_device_t* device = NULL;
  ...
  iree_hal_hip_deferred_work_queue_device_group_device_interface_t*
      device_interfaces[IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT];
  ...
  if (iree_status_is_ok(status) && device->params.stream_tracing) {
    ...
    iree_hal_hip_tracing_device_group_device_interface_t*
        tracing_device_interfaces[IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT];
    for (uint32_t i = 0; i < device_count; ++i) {
      status = iree_allocator_malloc(
          host_allocator,
...
iree_status_t iree_hal_hip_device_group_device_create(
  ...
  hipCtx_t hip_contexts[IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT];
  hipStream_t hip_dispatch_streams[IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT];
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_hip_device_group_device_create_internal(
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_hip_device_group_device_t* hip_device =
        iree_hal_hip_device_group_device_cast(*out_device);
...
iree_hal_hip_device_group_device_create_command_buffer_internal(
  ...
  iree_hal_command_buffer_t*
      buffers[IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT];
  memset(buffers, 0x00,
  ...
  if (IREE_UNLIKELY(!iree_status_is_ok(status))) {
    for (uint32_t i = 0; i < IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT; ++i) {
      if (buffers[i]) {
...
static iree_status_t iree_hal_hip_device_group_device_create_executable_cache(
  ...
  hipCtx_t contexts[IREE_HAL_HIP_MAX_DEVICE_GROUP_DEVICE_COUNT];
