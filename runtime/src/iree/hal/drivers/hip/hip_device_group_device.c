...
static iree_status_t iree_hal_hip_device_group_device_initialize_internal(
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_hip_cleanup_thread_initialize(host_allocator, symbols,
...
static iree_status_t iree_hal_hip_device_group_execute_now(
  ...
  for (iree_host_size_t i = 0;
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_hal_hip_semaphore_get_hip_event(
                ...
                data->device->device_contexts[idx].device_event_pool, false,
                &event));
  ...
  for (iree_host_size_t i = 0; i < data->signal_semaphore_list.count; ++i) {
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_hal_hip_semaphore_get_hip_event(
                ...
                data->device->device_contexts[idx].device_event_pool, true,
                &event));
    if (!event) {
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_hip_event_pool_acquire(
              data->device->device_contexts[idx].device_event_pool, 1, &event));
...
static iree_status_t iree_hal_hip_device_group_make_callback_data(
    iree_hal_hip_device_group_device_t* device, iree_allocator_t host_allocator,
    iree_arena_block_pool_t* block_pool,
    iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
  ...
  callback_data->command_buffer_count = command_buffer_count;
  callback_data->command_buffers = (iree_hal_command_buffer_t**)callback_ptr;
  memcpy(callback_data->command_buffers, command_buffers, command_buffers_size);
  ...
  if (iree_status_is_ok(status) && binding_table_elements_size > 0) {
    ...
    for (iree_host_size_t i = 0; i < command_buffer_count; ++i) {
      ...
      iree_hal_buffer_binding_table_t* target_table =
          &callback_data->binding_tables[i];
...
static iree_status_t iree_hal_hip_device_group_device_queue_execute(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0,
      iree_hal_hip_device_group_make_callback_data(
  ...
  for (iree_host_size_t i = 0;
    ...
    status = iree_hal_hip_semaphore_notify_work(
        ...
        device->device_contexts[nidx].device_event_pool,
