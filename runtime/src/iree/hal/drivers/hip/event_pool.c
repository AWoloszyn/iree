...
iree_status_t iree_hal_hip_event_pool_allocate(
  ...
  *out_event_pool = NULL;
  ...
  iree_status_t status = IREE_HIP_RESULT_TO_STATUS(
      symbols,
      hipCtxPushCurrent(device_context),
      "hipCtxPushCurrent");
...
iree_status_t iree_hal_hip_event_pool_acquire(
  ...
  if (remaining_count > 0) {
    ...
    iree_status_t status = IREE_HIP_RESULT_TO_STATUS(
        event_pool->symbols,
        hipCtxPushCurrent(event_pool->device_context),
        "hipCtxPushCurrent");
    if (iree_status_is_ok(status)) {
      ...
      if (iree_status_is_ok(status) && !iree_status_is_ok(cleanup_status)) {
        ...
        iree_hal_hip_event_pool_release_event(event_pool, from_pool_count + remaining_count,
                                                out_events);
      }
