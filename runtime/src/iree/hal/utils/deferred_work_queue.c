...
static void iree_hal_deferred_work_queue_completion_list_deinitialize(
  ...
  while (head) {
    if (head->created_event) {
      symbols->_vtable->destroy_native_event(symbols, head->native_event);
    }
    list->head = list->head->next;
    iree_allocator_free(host_allocator, head);
  }
...
// This replaces the use of *LaunchHostFunc, which causes the stream to block
// and wait for the CPU work to complete. It also picks up completed
// events with significantly less latency than *LaunchHostFunc.
...
iree_status_t iree_hal_deferred_work_queue_create(
  ...
  iree_hal_deferred_work_queue_working_area_initialize(host_allocator, symbols,
                                                       working_area);

  iree_hal_deferred_work_queue_completion_area_t* completion_area =
      &actions->completion_area;
  iree_hal_deferred_work_queue_completion_area_initialize(
      host_allocator, symbols, completion_area);
...
void iree_hal_deferred_work_queue_destroy(
  ...
  iree_hal_deferred_work_queue_completion_area_deinitialize(
      &work_queue->completion_area, work_queue->symbols,
  ...
  work_queue->symbols->_vtable->destroy(work_queue->symbols);
...
static iree_status_t iree_hal_deferred_work_queue_issue_execution(
  ...
  for (iree_host_size_t i = 0; i < action->event_count; ++i) {
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, symbols->_vtable->wait_native_event(
                symbols, symbols->_vtable->native_event_from_wait_event(
                             symbols, action->wait_events[i])));
  }
  ...
  for (iree_host_size_t i = 0; i < action->payload.execution.count; ++i) {
    ...
    if (iree_hal_deferred_command_buffer_isa(command_buffer)) {
      ...
      iree_hal_command_buffer_mode_t mode =
          ...
          (iree_hal_buffer_binding_table_is_empty(binding_table)
               ...
               : 0);
      IREE_RETURN_AND_END_ZONE_IF_ERROR(
          z0, action->symbols->_vtable->create_command_buffer_for_deferred(
                  action->symbols, mode, IREE_HAL_COMMAND_CATEGORY_ANY,
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, action->symbols->_vtable->submit_command_buffer(symbols,
                                                            command_buffer));
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, symbols->_vtable
                ->semaphore_acquire_timepoint_device_signal_native_event(
                    symbols, action->signal_semaphore_list.semaphores[i],
                    action->signal_semaphore_list.payload_values[i], &event));

    // Record the event signaling in the dispatch stream.
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, symbols->_vtable->record_native_event(symbols, event));
  ...
  if (IREE_UNLIKELY(!completion_event)) {
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, symbols->_vtable->create_native_event(symbols, &completion_event));
    created_event = true;
  }

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, symbols->_vtable->record_native_event(symbols, completion_event));
    ...
    if (action->state == IREE_HAL_QUEUE_ACTION_STATE_ALIVE) {
      for (iree_host_size_t i = 0; i < action->wait_semaphore_list.count; ++i) {
        ...
        if (value >= values[i]) {
          ...
          iree_hal_semaphore_list_erase(&action->wait_semaphore_list,
                                                 i);
          --i;
          continue;
        }
        ...
        if (!action->symbols->_vtable->acquire_host_wait_event(
                action->symbols, semaphores[i], values[i], &wait_event)) {
          action->is_pending = true;
          break;
        }
...
static void iree_hal_deferred_work_queue_worker_process_completion(
  ...
  while (true) {
    ...
    if (IREE_LIKELY(iree_status_is_ok(status))) {
      ...
      status = actions->symbols->_vtable->synchronize_native_event(
          actions->symbols, entry->native_event);
    ...
    if (IREE_UNLIKELY(entry->created_event)) {
      status = iree_status_join(status,
                                actions->symbols->_vtable->destroy_native_event(
                                    actions->symbols, entry->native_event));
    }
    iree_allocator_free(actions->host_allocator, entry);
  }
...
static int iree_hal_deferred_work_queue_completion_execute(
  ...
  iree_status_t status =
      actions->symbols->_vtable->bind_to_thread(actions->symbols);
  if (IREE_UNLIKELY(!iree_status_is_ok(status))) {
    iree_hal_deferred_work_queue_fail(actions, status);
    iree_status_ignore(status);
  }
